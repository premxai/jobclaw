...
class HealthTracker:
    ...
    def save(self) -> None:
        ...
        with open(self.path, "w", encoding="utf-8") as f:
            json.dump({
                "_comment": "Scraper health metrics for monitoring",
                "_schema_version": 1,
                **self._data
            }, f, indent=2)
    ...
    def get_alerts(self) -> list[dict]:
        ...
        if total > 0:
            ...
            if error_rate > 0.15:
                alerts.append({
                    "level": "critical",
                    "message": f"High error rate: {error_rate:.1%} ({total_errors} errors)"
                })
            elif error_rate > 0.05:
                alerts.append({
                    "level": "warning",
                    "message": f"Elevated error rate: {error_rate:.1%} ({total_errors} errors)"
                })
        ...
        if retry_size > 100:
            alerts.append({
                "level": "warning",
                "message": f"Large retry queue: {retry_size} companies pending"
            })
        ...
        if last_run:
            try:
                ...
                if hours_since > 6 and self._data.get("new_jobs", 0) == 0:
                    alerts.append({
                        "level": "warning",
                        "message": f"No new jobs found in last {int(hours_since)} hours"
                    })
    ...
    def get_summary(self) -> str:
        ...
        lines = [
            ...
            f"Jobs: {self._data.get('jobs_found', 0)} found, "
            f"{self._data.get('new_jobs', 0)} new",
...
def format_discord_alert(alerts: list[dict]) -> Optional[str]:
    """

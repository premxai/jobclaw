...
PRIORITY_COMPANIES = [
    ...
    {"company": "Perplexity AI", "ats": "ashby", "slug": "Perplexity"},
    ...
    {"company": "Runway ML", "ats": "ashby", "slug": "Runway"},
    {"company": "Together AI", "ats": "greenhouse", "slug": "togetherai"},
    {"company": "Stability AI", "ats": "greenhouse", "slug": "stabilityai"},
    # High-Growth Tech
    ...
    async def _fetch_one(entry):
        async with sem:
            try:
                ...
                if failure:
                    log(
                        f"[priority] {entry['company']} ({entry['ats']}) {failure.get('category')}:{failure.get('status_code') or 'n/a'}",
                        "WARN",
                    )
                return result or []
            except Exception as e:

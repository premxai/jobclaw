...
def _tokenize(text: str) -> set[str]:
    ...
    stopwords = {
        ...
        'we', 'you', 'your', 'our', 'their', 'its', 'as', 'if', 'not', 'no',
        'all', 'any', 'each', 'every', 'such', 'what', 'which', 'who', 'whom',
        'more', 'most', 'other', 'some', 'about', 'into', 'through', 'also',
    }
    ...
    def merge_duplicates(self, clusters: list[list[dict]]) -> int:
        ...
        try:
            for cluster in clusters:
                ...
                cluster.sort(
                    key=lambda x: (x["has_salary"], x["description_len"]),
                    reverse=True
                )
...
if __name__ == "__main__":
    ...
    for i, cluster in enumerate(clusters[:10]):
        ...
        for item in cluster:
            print(f"    [{item['source_ats']}] {item['title']} @ {item['company']} "
